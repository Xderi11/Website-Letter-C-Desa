...
if(isset($_POST['submit'])){
    ...
    // Query untuk memeriksa apakah NIK ada di database perubahan
    $query_check = "SELECT * FROM perubahan WHERE nik = '$nik'";
    $result_check = mysqli_query($connect, $query_check);

    if(mysqli_num_rows($result_check) > 0){
        // Data ditemukan, arahkan ke halaman detail
        header("Location: detail.php?nik=$nik");
        exit();
    } else {
...
<div class="content-wrapper">
    <section class="content-header">
        <h1>Pengecekan NIK</h1>
        <ol class="breadcrumb">
            <li><a href="../../dashboard/"><i class="fa fa-tachometer-alt"></i> Dashboard</a></li>
            <li class="active">Laporan Kepemilikan</li>
        </ol>
    </section>
                        ...
                        <div class="card-content">
                            <div class="card-body">
                                <form action="index.php" method="post"> 
                                    ...
                                    <label style="font-weight: 700;"><i class="fas fa-id-card"></i> NIK (NOMOR INDUK KEPENDUDUKAN) </label>
                                    <input type="text" class="form-control form-control-md" name="nik" placeholder="Masukkan NIK anda..." required>
                                    <br>
                                    <button type="submit" name="submit" class="btn btn-info btn-md"><i class="fas fa-search"></i> CEK NIK</button>
                                </form>
                            </div>
                        </div>
                    </div>
